...
A proposal is untrusted input crossing the PLAN-to-supervisor air gap. PLAN may
create files only at `.proposals/*.json`. The supervisor must unlink each staged
file immediately after reading its bytes, before validation or compilation.
