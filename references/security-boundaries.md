...
### PLAN
Read-only target-workpiece inspection within the TaskEnvelope's exact path
authority. The only PLAN write is one proposal file whose path matches
`^\.proposals\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\.json$`. No shell, compiler
execution, git, patching, network, or sealing. The supervisor unlinks the staged
proposal immediately after reading its bytes.
